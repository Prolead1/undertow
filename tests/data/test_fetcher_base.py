...
def test_request_paces_every_call_not_just_each_chunk(
    ...
    A chunk that fans out into several RPC calls (gas: ``eth_feeHistory`` + header batches)
    would otherwise fire them back-to-back and burst past a provider's CUPS bucket. The
    first request has nothing to wait for; the second must be paced.
    """
    sleeps: list[float] = []
    ...
    assert 0.0 <= sleeps[0] <= 0.5  # waits at most min_interval_s
